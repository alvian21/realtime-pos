...
export async function createUser(prevState: AuthFormState, formData: FormData | null) {
    ...
    if (validateFields.data.avatar_url instanceof File) {
        const { errors, data } = await uploadFile('images', 'users', validateFields.data.avatar_url);
...
export async function updateUser(prevState: AuthFormState, formData: FormData | null) {
    ...
    if (validateFields.data.avatar_url instanceof File) {
        ...
        const { errors, data } = await uploadFile('images', 'users', validateFields.data.avatar_url, oldAvatarUrl.split('/images/')[1]);
